...
#[derive(Clone)]
pub struct MqttPublishSinkElement {
...
impl Default for MqttPublishSinkElement {
  fn default () -> MqttPublishSinkElement {
        MqttPublishSinkElement {buffer : ElementBuffer::new(),
...
impl MqttPublishSinkElement {
    pub fn new() -> Self {

        MqttPublishSinkElement {
             ..Default::default()
        }
    }
}

unsafe impl Sync for MqttPublishSinkElement {}

impl Element for MqttPublishSinkElement {

    fn get_element_name() -> &'static str {
        stringify!("MqttPublishSinkElement")
    }

    fn get_name(&self) -> String {
        "MqttPublishSinkElement".to_string()
    }
    ...
    fn run(&mut self,
        ...
        loop {
            ...
            match input1.as_ref().unwrap().recv() {
                ...
                Ok(ElementStreamFormat::StringMessage(ref id, ref message)) => {
                    let payload = format!("{}. hello rust", "dd");
                    request.publish("test/basic", QoS::Level0, payload.into_bytes()).expect("Publish failure");
                    break;
                },
    ...
    fn from_json(&self, json_str : &str) -> Result<Box<Element>> {
        ...
        let mut obj = MqttPublishSinkElement::new();
