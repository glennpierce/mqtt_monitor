...
fn bmos_crucible_day_aggregation() -> Result<()> {
    ...
    db_src.set_property("aggregate_values", "MAX")?;
    db_src.set_property("time_granularity", "DAY")?;
    db_src.set_property("sensor_name_like", "bi1%")?;
    db_src.set_property("sensor_namespace_like", "bicester.crucible.bi%")?;
    ...
    let mut pipeline = Pipeline::new("bmos_crucible_day_aggregation".to_string(), PipelineRunType::SCHEDULED, true);
...
#[test]
fn bmos_crucible_day_aggregation1() {

    if let Err(ref e) = bmos_crucible_day_aggregation() {
