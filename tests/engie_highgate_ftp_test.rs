...
fn engie_highgate_london_ftp_test() -> Result<()> {
    ...
    db_src.set_property("filter", "/HighGateMH_London_%")?;

    let mut csv_file_element = CsvFileElement::new();
    csv_file_element.set_property("datetime_format", "%d/%m/%Y  %H:%M:%S")?;
    csv_file_element.set_property("filename_prefix", "HighGateMH_London_")?;
    ...
    sftp_sink_element.set_property("user", "HighGateMH")?;
    //sftp_sink_element.set_property("password", "P97H2M28757UxoT")?;
    sftp_sink_element.set_property("key", "id_highgatemh_london_rsa")?;
    sftp_sink_element.set_property("working_directory", "London")?;
    sftp_sink_element.set_property("fake", "false")?;

    let mut pipeline = Pipeline::new("engie_highgate_london".to_string(), PipelineRunType::SCHEDULED, true);
...
#[test]
fn engie_highgate_london_ftp_test1() {

    pretty_env_logger::init().unwrap();

    println!("running engie highgate_london ftp");
    if let Err(ref e) = engie_highgate_london_ftp_test() {
