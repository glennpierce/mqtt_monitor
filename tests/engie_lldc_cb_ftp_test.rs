...
fn engie_lldc_cb_ftp_test() -> Result<()> {
    ...
    sftp_sink_element.set_property("working_directory", "LLDC_CB")?;
    sftp_sink_element.set_property("fake", "false")?;

    let mut pipeline = Pipeline::new("engie_lldc_cb".to_string(), PipelineRunType::SCHEDULED, true);
...
#[test]
fn engie_lldc_cb_ftp_test1() {

    pretty_env_logger::init().unwrap();

    println!("running engie lldc_cb ftp");
    if let Err(ref e) = engie_lldc_cb_ftp_test() {
