...
fn engie_c3_ftp_test() -> Result<()> {
    ...
    sftp_sink_element.set_property("user", "GENIETest1")?;
    //sftp_sink_element.set_property("password", "P97H2M28757UxoT")?;
    sftp_sink_element.set_property("key", "id1_ftp_rsa")?;
    sftp_sink_element.set_property("working_directory", "PlymouthGenie")?;
    sftp_sink_element.set_property("fake", "false")?;

    let mut pipeline = Pipeline::new("engie_c3".to_string(), PipelineRunType::SCHEDULED, true);
...
#[test]
fn engie_c3_ftp_test1() {

    pretty_env_logger::init().unwrap();

    println!("running engie c3 ftp");
    if let Err(ref e) = engie_c3_ftp_test() {
