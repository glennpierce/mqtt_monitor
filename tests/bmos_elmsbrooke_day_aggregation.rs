...
fn bmos_elmsbrooke_day_aggregation_debug() -> Result<()> {
    ...
    db_src.set_property("aggregate_values", "MIN_MAX_DIFF")?;
    ...
    let mut bmos_table_sink_element = BmosTableSinkElement::new();
    ...
    let mut pipeline = Pipeline::new("bmos_elmsbrooke_day_aggregation".to_string(), PipelineRunType::SCHEDULED, true);
...
fn bmos_elmsbrooke_day_aggregation_release() -> Result<()> {
...
#[test]
fn bmos_elmsbrooke_day_aggregation1() {

    if let Err(ref e) = bmos_elmsbrooke_day_aggregation_release() {
