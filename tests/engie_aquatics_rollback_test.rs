...
fn engie_acquatics_ftp_backtrack_test() -> Result<()> {
    ...
    //db_src.set_property("start_time", "2017-03-14T00:00:00+00:00")?;

    println!("now: {}", Utc::now());

    let start = Utc::now() - Duration::seconds(3600 * 2); // 2 hours

    println!("start: {}", start);

    db_src.set_property("start_time", &start.to_rfc3339())?;
    db_src.set_property("offset_from_now", "3600")?;   // 1 hour
    db_src.set_property("backtrack", "3600")?;   // 1 hour
    db_src.set_property("tables", "HISTORYNUMERICTRENDRECORD,HISTORYBOOLEANTRENDRECORD")?;
    db_src.set_property("filter", "/AQUATICS%")?;

    let mut csv_file_element = CsvFileElement::new();
    csv_file_element.set_property("datetime_format", "%d/%m/%Y  %H:%M:%S")?;
    csv_file_element.set_property("filename_prefix", "Aquatics_")?;
    csv_file_element.set_property("max_number_of_lines", "0")?;
    ...
    sftp_sink_element.set_property("user", "LLDC")?;
    //sftp_sink_element.set_property("password", "P97H2M28757UxoT")?;
    sftp_sink_element.set_property("key", "id_aquatics_rsa")?;
    sftp_sink_element.set_property("working_directory", "Aquatics")?;
    sftp_sink_element.set_property("fake", "true")?;

    let mut pipeline = Pipeline::new("engie_aquatics_backtrack".to_string(), PipelineRunType::SCHEDULED, true);
...
#[test]
fn engie_acquatics_ftp_backtrack_test1() {

    pretty_env_logger::init().unwrap();

    println!("running engie aquatics ftp backtrack");
    if let Err(ref e) = engie_acquatics_ftp_backtrack_test() {
